...
def main():
    ...
    #cf_matrix = confusion_matrix(y, pred.round().reshape(-1).detach())
    #print(cf_matrix)
    #tf.expand_dims(y, -1).shape.as_list()
    #tf.enable_eager_execution()
    #print(tf.shape(y).numpy(), tf.shape(pred).numpy())
    #y1 = tf.reshape(y,(-1,1))
    #pred1 = tf.reshape(pred, (-1, 1))
    #cf_matrix = tf.confusion_matrix(y1, pred1)
    #sess = tf.Session()
    ...
    from sklearn.metrics import accuracy_score, precision_score, f1_score, recall_score, brier_score_loss, \
        roc_auc_score, roc_curve, auc, classification_report, plot_confusion_matrix

    #y_pred_svm = svc.predict(X_train_sc).ravel()
    fpr_svm, tpr_svm, thresholds_svm = roc_curve(train_y, np.rint(pred_train))
    ...
    plt.plot(fpr_svm, tpr_svm, label='rbfn (area = {:.3f})'.format(auc_svm))
